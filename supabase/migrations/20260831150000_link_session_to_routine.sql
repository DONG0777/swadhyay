...
begin
  ...
  if p_routine_id is not null then
    select *
    into v_routine
    from public.community_routines
    where id = p_routine_id;

    if not found then
      raise exception 'Community routine not found.';
    end if;

    if v_routine.place_id <> p_place_id then
      raise exception 'Routine does not belong to this community place.';
    end if;

    if not v_routine.is_active then
      raise exception 'This routine is not active.';
    end if;
  end if;
...
drop function if exists public.create_community_session(
  uuid,
  text,
  text,
  text,
